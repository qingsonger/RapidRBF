...
use rapidrbf_faer_control::EventKind;
...
unsafe fn pack_rhs_imp<T: Copy>(dst: *mut T, src: *const (), depth: usize, stride: usize, nr: usize, rs: isize, cs: isize) {
	for i in 0..depth {
		rapidrbf_faer_control::checkpoint(EventKind::Packing, nr);
		unsafe {
...
pub unsafe fn call_microkernel(
	...
	rapidrbf_faer_control::checkpoint(
		EventKind::MacroKernel,
		micro.depth.saturating_mul(nrows).saturating_mul(ncols),
	);
	unsafe {
		core::arch::asm! {
			"call r10",
...
unsafe fn pack_lhs(pack: unsafe extern "C" fn(), milli: &MillikernelInfo, row_chunk: usize, packed_lhs: *mut (), lhs: *const (), sizeof: usize) {
	rapidrbf_faer_control::checkpoint(
		EventKind::Packing,
		milli.micro.depth.saturating_mul(row_chunk),
	);
	unsafe {
		{
...
pub fn workspace_layout(dtype: DType, instr: InstrSet) -> WorkspaceLayout {
	let sizeof = match dtype {
		DType::F32 => 4,
		DType::F64 => 8,
		DType::C32 => 8,
		DType::C64 => 16,
	};
	let nr = match (instr, dtype) {
		(InstrSet::Avx256, _) => 4,
		(InstrSet::Avx512, DType::F64) => 8,
		(InstrSet::Avx512, _) => 4,
	};
	let cache = *cache::CACHE_INFO;
	let l3 = cache[2].cache_bytes / sizeof;
	let lhs_pages = (l3.next_multiple_of(16) * sizeof).div_ceil(size_of::<Page>());
	let rhs_pages = (l3.next_multiple_of(nr) * sizeof).div_ceil(size_of::<Page>());
	WorkspaceLayout {
		bytes: (lhs_pages + rhs_pages)
			.checked_mul(size_of::<Page>())
			.expect("private-gemm workspace layout overflow"),
		align: align_of::<Page>(),
	}
}
...
impl OwnedWorkspace {
	fn new(layout: WorkspaceLayout) -> Self {
		let layout = std::alloc::Layout::from_size_align(layout.bytes, layout.align)
			.expect("valid private-gemm workspace layout");
		rapidrbf_faer_control::reserve_transient(layout.size(), layout.align());
		let ptr = unsafe { std::alloc::alloc(layout) };
		let Some(ptr) = core::ptr::NonNull::new(ptr) else {
			rapidrbf_faer_control::release_transient(layout.size(), layout.align());
...
impl Drop for OwnedWorkspace {
	fn drop(&mut self) {
		...
		rapidrbf_faer_control::release_transient(
			self.layout.size(),
			self.layout.align(),
		);
	}
...
pub unsafe fn gemm(
	...
	let required = workspace_layout(dtype, instr);
	let owned_workspace = if workspace.is_null() {
		Some(OwnedWorkspace::new(required))
	} else {
		assert!(workspace_len >= required.bytes);
		assert_eq!((workspace as usize) % required.align, 0);
		None
	};
	let workspace = owned_workspace
		.as_ref()
		.map(|owned| owned.ptr.as_ptr())
		.unwrap_or(workspace);
	rapidrbf_faer_control::backend_entry();
	let mem = unsafe {
		core::slice::from_raw_parts_mut(
			workspace as *mut core::mem::MaybeUninit<Page>,
			lhs_size + rhs_size,
		)
	};
	{
