...
pub fn spicy_matmul_scratch<T: ComplexField>(
	...
	if const {
		...
		if std::is_x86_feature_detected!("avx2")
			&& std::is_x86_feature_detected!("fma")
		{
			use private_gemm_x86::{DType, InstrSet};
			let dtype = const {
				if T::IS_NATIVE_F64 {
					DType::F64
			...
			let instr = if std::arch::is_x86_feature_detected!("avx512f") {
				InstrSet::Avx512
			} else {
				InstrSet::Avx256
			};
			let workspace = private_gemm_x86::workspace_layout(dtype, instr);
			return StackReq::new_aligned::<u8>(
				workspace.bytes,
				workspace.align,
			);
		}
	...
	if const {
		...
		if let Some(feat) = feat {
			...
			let workspace_layout =
				private_gemm_x86::workspace_layout(dtype, feat);
			let (workspace, _) = unsafe {
				stack.make_aligned_raw::<u8>(
					workspace_layout.bytes,
					workspace_layout.align,
				)
			};
			let mut C = C;
			let mut A = A;
